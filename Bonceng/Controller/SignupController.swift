...
class SignupController: UIViewController {
    ...
    @objc func handleSignUp() {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { result, error in
            if let error = error {
                print("DEBUG: failed ro register user with error \(error.localizedDescription)")
                return
            }
            ...
            Database.database().reference().child("users").child(uid).onDisconnectUpdateChildValues(values) { error, ref in
